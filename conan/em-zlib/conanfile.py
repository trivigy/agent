            ...
            with tools.chdir("_build"):
                if not tools.os_info.is_windows:
                    ...
                    with tools.environment_append(env.vars):
                        ...
                        self.run("emconfigure ../configure")
                        self.run("emmake make -j {}".format(tools.cpu_count()))

                else:
                    cmake = CMake(self)
                    self.run('emcmake cmake "%s" %s' % (self.source_folder, cmake.command_line))
                    self.run('emcmake cmake --build . %s' % cmake.build_config)
